...
def prediction(index, model, centers_X, tans_X):
    ...
    for i in range(4):
        ...
        rad = tans_X[index][i] * math.pi
        end_pt = (int(centers_X[index][2*i] * temp.shape[0] + 300 * math.cos(rad)), int(centers_X[index][2*i+1] * temp.shape[1] + 300 * math.sin(rad)))
    ...
    rad = tan[index][0] * math.pi
    end_pt = (int(center[index][0] * temp.shape[0] + 300 * math.cos(rad)),
              int(center[index][1] * temp.shape[1] + 300 * math.sin(rad)))
...
def loading():
    ...
    return model, centers_X, tans_X




if __name__ == "__main__":
    model, centers_X, tans_X = loading()

    # while(1):
    #     index = input(int())
    #     prediction(index, model, centers_X, tans_X)
    prediction(0, model, centers_X, tans_X)
    prediction(5, model, centers_X, tans_X)
    prediction(10, model, centers_X, tans_X)
    prediction(15, model, centers_X, tans_X)
    prediction(20, model, centers_X, tans_X)
